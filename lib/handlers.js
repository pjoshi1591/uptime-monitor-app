                                        ...
                                        _data.delete('checks',checkId, (err) => {
                                            ...
                                            if(checksDeleted == checksToDelete) {
                                                if(!deletionErrors) {
                                                    ...
                                                    callback(500,{'Error' : 'Error encountered'});
                                                }
                                            }
                                        });
                                    });
        ...
        _data.read('users', phone, (err,userData) => {
            if(!err && userData) {
                ...
                if(hashedPassword == userData.hashedPassword) {
                    // If valid, create a new token with a random name. Set expiration date 1 hour hour in the future
        ...
        _data.read('tokens',token, (err,tokenData) => {
            if(!err && tokenData) {
                ...
                _data.read('users',userPhone, (err,userData) => {
                    if(!err && userData) {
                        const userChecks = typeof(userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : [];
                        // Verify that the user has less than the number of max-cheks per user
                                ...
                                if(!err) {
                                    ...
                                    _data.update('users',userPhone,userData, (err) => {
                                        if(!err) {
                                            ...
                                            callback(500,{'Error' : 'Could noy update user with the new check'})
                                        }
                                    })
                                } else {
    ...
    if(id) {
        ...
        _data.read('checks',id, (err,checkData) => {
            if(!err && checkData) {
                // Get token from the headers
                const token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
                // Verify that the given token is valid and belong to the use who created the check
                handlers._tokens.verifyToken(token,checkData.userPhone, (tokenIsValid) => {
                    if(tokenIsValid) {
                        // Retutn the check data
                        ...
                        if(tokenIsValid) {
                            ...
                            _data.update('checks',id,checkData,(err) => {
                                if(!err) {
                                    ...
                                    callback(500, {'Error' : 'Could not save the updates'})
                                }
                            });
                        } else {
                            callback(403);
